...
jq -r '.[] | if has("headers") then "curl --trace - \(.headers | to_entries | map("-H \"",.key, ":", .value, "\" ") | add) --request \(.method) $swserver\"\(.headers.Location)\" | curl-trace-parser --blueprint | iconv -t iso-8859-1 -f utf-8 > log/\(.id).log" else "" end' < ../Example.json > output/Example.sh
